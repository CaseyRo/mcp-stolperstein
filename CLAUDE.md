...
`mcp-stolperstein` — a FastMCP server that captures and recalls experiential knowledge ("Knowledge Units" / KUs) for AI coding agents. Internal model is a superset of Mozilla AI's upstream `cq` schema; strict-mode serializer (`to_cq_json_strict()`) emits upstream-valid payloads, `to_cq_json_rich()` emits the full internal shape with Stolperstein extensions. Upstream discussion of the extensions we propose: [mozilla-ai/cq#286](https://github.com/mozilla-ai/cq/discussions/286). Registry of extensions: `docs/cq-extensions.md`.
...
uv run mcp-stolperstein              # run the server (stdio or HTTP per TRANSPORT env)
uv run mcp-stolperstein migrate      # apply pending DB migrations + exit
uv run mcp-stolperstein prune-backups [--confirm]   # clean .bak-pre-v<N> files
uv run mcp-stolperstein detect-emergent             # run emergent-signal aggregation
...
Tests force `TRANSPORT=stdio`, inject a deterministic zero-signing-key via `MCP_STOLPERSTEIN_SIGNING_KEY`, and use `NoOpEmbeddings`. Do not rely on sentence-transformers being loaded in unit tests.
...
Stolperstein extensions (all local-only, all documented in `docs/cq-extensions.md`): `evidence.severity`, `evidence.contributing_orgs`, `context.environment`, top-level `kind`/`status`/`staleness_policy`/`related[]`/`owner_org`, `provenance.graduation_history`/`emergent`.
...
```python
# src/stolperstein/migrations/m000N_your_slug.py
version = N
...
- If you add a field that would ship on the wire, decide between strict upstream and Stolperstein extension: extensions go in `models.py` AND get a row in `docs/cq-extensions.md`.

## Plugin

`plugin/stolperstein/` is a Claude Code plugin (registered via `.claude-plugin/marketplace.json`) that wraps these tools with `SKILL.md` (usage guidance + what's strict-CQ vs extension) and a real `hooks.json` manifest firing the three Python handlers above. See `plugin/stolperstein/SKILL.md` for the `STOLPERSTEIN_HOOKS_DISABLED` escape hatch and per-project overrides.
