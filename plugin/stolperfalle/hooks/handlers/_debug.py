...
Set `STOLPERFALLE_HOOKS_DEBUG=1` and every handler appends one JSON line
per decision to `$TMPDIR/stolperfalle-hooks-debug.jsonl` — including the
...
def _enabled() -> bool:
    val = os.environ.get("STOLPERFALLE_HOOKS_DEBUG", "").strip().lower()
    return val in {"1", "true", "yes", "on"}


def trace_path() -> Path:
    return Path(tempfile.gettempdir()) / "stolperfalle-hooks-debug.jsonl"
