#!/usr/bin/env python3
"""Stop hook — nudges `/stolperfalle:reflect` if the session was
substantive: ≥ STOLPERFALLE_REFLECT_THRESHOLD tool-call turns AND at
least one non-zero bash exit OR one flag()/confirm() call.

Two modes:

- **Default**: emits a one-line nudge telling the user to invoke
  `/stolperfalle:reflect` manually. The skill then drives the model
  through the reflect → propose flow.
- **Opt-in via STOLPERFALLE_REFLECT_VIA_HOOK=true**: skips the nudge,
...
Also emits a one-time "Stolperfalle unreachable" notice if any prior hook
...
def _threshold() -> int:
    try:
        return int(os.environ.get("STOLPERFALLE_REFLECT_THRESHOLD", _DEFAULT_THRESHOLD))
    except ValueError:
...
def _reflect_via_hook_enabled() -> bool:
    ...
    nudge-based flow.
    """
    val = os.environ.get("STOLPERFALLE_REFLECT_VIA_HOOK", "").strip().lower()
...
def _consume_unreachable_notice(sid: str) -> str | None:
    ...
    try:
        ...
        pass
    return (
        "Stolperfalle was unreachable during this session — "
...
async def _run() -> int:
    ...
    _emit_system_message(
        notice, "Run `/stolperfalle:reflect` to capture session learnings."
    )
    return 0
