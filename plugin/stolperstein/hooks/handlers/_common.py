...
def disabled(hook_name: str) -> bool:
    """True if `hook_name` is listed in STOLPERSTEIN_HOOKS_DISABLED."""
    raw = os.environ.get("STOLPERSTEIN_HOOKS_DISABLED", "").strip()
...
def unreachable_marker(sid: str) -> Path:
    return Path(tempfile.gettempdir()) / f"stolperstein-unreachable-{sid}"
