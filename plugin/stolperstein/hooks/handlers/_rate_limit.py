"""Rate limit + per-KU dedupe for Stolperstein hook handlers.
...
- **Cooldown**: no hook of the given type may inject more than once per
  `STOLPERSTEIN_HOOK_COOLDOWN_S` seconds (default 30).
...
def _cooldown_s() -> int:
    try:
        return int(os.environ.get("STOLPERSTEIN_HOOK_COOLDOWN_S", _DEFAULT_COOLDOWN_S))
    except ValueError:
